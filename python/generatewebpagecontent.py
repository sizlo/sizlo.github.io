...
def getSmallImage(regularPath):
    pathWithoutExtension = os.path.splitext(regularPath)[0]
    extension = os.path.splitext(regularPath)[1]
    return pathWithoutExtension + "_small" + extension

def getFullImage(regularPath):
    pathWithoutExtension = os.path.splitext(regularPath)[0]
    extension = os.path.splitext(regularPath)[1]
    return pathWithoutExtension + "_full" + extension

def getImageHtml(path, isCover=False):
    # Link to the full size image
    fullSizePath = getFullImage(path)
    # None cover images should use the small version
    smallPath = getSmallImage(path)
    if not isCover:
        path = smallPath
...
def generateAlbumPage(albumData, path):
    ...
    htmlContent += "      " + getImageHtml(albumData["coverimagepath"], isCover=True) + "\n"
    htmlContent += "    </div>\n"
    htmlContent += "\n"
...
def generateBuildHtmlForHome(buildData, relativepath):
    ...
    htmlContent += "    <div class=\"img\" style=\"background-image:url('" + relativepath + "/" + getSmallImage(buildData["coverimagepath"]) + "');\"></div>\n"
    htmlContent += "  </a>\n"
    htmlContent += "</div>\n"
    return htmlContent
